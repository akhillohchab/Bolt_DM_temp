 ...
 */
 ...
 public class InstanceBuilderASRPROSPOS implements InstanceBuilderWrapper {	 
	...
	public Instances buildInstance(SentenceData input){
		...
		*/
		...
		//create all the attributes and add them to the vector
		 ...
		 Attribute f0max = new Attribute ("F0MAX");//numeric\n";
		 attributes.addElement(f0max);		 
		 
		 Attribute f0stdev = new Attribute ("F0STDEV");//numeric\n";
		 attributes.addElement(f0stdev);		 

		 Attribute engmean = new Attribute ("ENGMEAN"); //numeric\n";
		 attributes.addElement(engmean);
		 
		 Attribute engmin = new Attribute ("ENGMIN"); //numeric\n";
		 attributes.addElement(engmin);
		 
		 Attribute engmax = new Attribute ("ENGMAX"); //numeric\n";
		 attributes.addElement(engmax);
		 		 
		 Attribute engstdev = new Attribute ("ENGSTDEV");//numeric\n";
		 attributes.addElement(engstdev);
		 
		 Attribute vcd2tot = new Attribute ("VCD2TOT"); //numeric\n";
		 attributes.addElement(vcd2tot);
	     ...
	     for (int i = 0; i < input.getWordsCurrentUtt().size(); i++) {		
	    	 Instance inst = new Instance(22);
	    	  ...
	    	  //Set POS tags	    	   
	    	    ...
	    	    inst.setValue(total_dur, input.getDuration().get(i));
	    	    ...
	    	    inst.setValue(f0stdev, input.getF0stdevWords().get(i));
	    	    inst.setValue(engmean, input.getEngmeanWords().get(i));
	    	    inst.setValue(engmin, input.getEngminWords().get(i));
	    	    inst.setValue(engmax, input.getEngmaxWords().get(i));
	    	    inst.setValue(engstdev, input.getEngstdevWords().get(i));
	    	    inst.setValue(vcd2tot, input.getVcd2totWords().get(i));
				m_Data.add(inst);
			}		     
